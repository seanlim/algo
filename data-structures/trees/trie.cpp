...
// Remove a node
bool remove(Node *node, string s, int index,
            vector<Node *> stk = vector<Node *>(), Node *fork = new Node(),
            char split_char = char()) {
  if (s.length() == index) {
    // Base Case
  ...
  } else if (node->children[s[index]] == NULL)
    return false;
  else {
    // Recursive case
    ...
    return remove(node->children[s[index]], s, index + 1, stk_t, fork_t,
                  split_t);
  }
}
